...
/* ─────────────────────────────────────────────────────────────
   One form, two doors. /day-use/booking asks the guest to pick a
   date; a fixed-date variant (an event day) states it instead and
   explains what the day is. Everything past step 1 — pricing,
   validation, the "day-use" service, the emails — is identical,
   because an event registration IS a day-use booking.
   ───────────────────────────────────────────────────────────── */
      ...
      {highlights && highlights.length > 0 && (
        ...
        >
          {highlights.map((line) => (
            <li
              key={line}
              className="text-[0.875rem] leading-relaxed flex gap-2.5"
              style={{ color: MUTED }}
            >
              <span
                aria-hidden="true"
...
/* ── Standing banner: postponement, weather, anything the guest must
      read before filling in three steps. ── */
function NoticeBanner({ title, body }: { title: string; body: string }) {
  return (
    ...
    >
      <p className="text-[0.875rem] font-[600]" style={{ color: "#b45309" }}>
        {title}
      </p>
      <p
...
/* ── Dead end, stated early. A fixed-date form whose date is gone or
      full must say so on arrival rather than let someone fill in three
      steps and meet the server's refusal. ── */
function ClosedNotice({
  rail,
  title,
  body,
}: {
  rail: DayUseBookingVariant["rail"];
  title: string;
  body: string;
}) {
  return (
    ...
    >
      <div className="w-full max-w-md text-center">
        <span className={cn(eyebrow, "block mb-4")} style={{ color: MUTED }}>
          {rail.eyebrow}
        </span>
        <h1
          className="font-[family-name:var(--font-raleway)] text-[1.75rem] font-[600] tracking-[-0.01em] mb-3"
          style={{ color: NAVY }}
        >
          {title}
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Button
            asChild
            className={cn(eyebrow, "rounded-full min-h-12 px-6")}
            style={{ background: SKY, color: NAVY }}
          >
            <Link href="/day-use/booking">Book another day</Link>
          </Button>
          <Button
            asChild
            variant="outline"
  ...
  if (fixedDateIsPast) {
    return (
      <ClosedNotice
        ...
        body="Registration for this day is closed. The beach is open every other day — pick one and we'll see you there."
      />
    );
  }

  if (fixedDateIsClosed) {
    return (
      <ClosedNotice
        rail={rail}
        title="This day is fully booked"
        body="We've reached capacity for this date. Message us and we'll let you know if anything opens up."
      />
    );
  }
      ...
      <div className="md:w-1/2 lg:w-[45%] flex flex-col justify-center px-4 sm:px-8 lg:px-14 py-8 md:py-12 pb-28 md:pb-12">
        <div className="w-full max-w-md mx-auto">
          ...
          <h1
            ...
            style={{ color: NAVY }}
          >
            {step === 1 ? variant.stepOneTitle : STEP_TITLES[step]}
          </h1>
          ...
          {notice && (
            <NoticeBanner title={notice.title} body={notice.body} />
          )}
                    ...
                    return (
                      <Field data-invalid={isInvalid}>
                        <FieldLabel
                          ...
                          style={{ color: NAVY }}
                        >
                          {fixedDate ? "Your day" : "Arrival date"}
                        </FieldLabel>
                        {fixedDate ? (
                          <FixedDatePanel
                            date={fixedDate}
                    ...
                    {date && (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3 mb-4">
                          ...
                          <button
                            type="button"
                            onClick={() => goToStep(fixedDate ? 2 : 1)}
