...
export default function PaymentCountdown({ deadline }: { deadline: string }) {
  ...
  if (remaining <= 0) {
    return (
      <div className="inline-flex items-center gap-2 rounded-full bg-[#FBE3E1] px-4 py-2 ring-1 ring-[#F1C0BB]">
        <Clock className="h-3.5 w-3.5 text-[#7E2A23]" strokeWidth={1.8} />
        <span className="font-[family-name:var(--font-raleway)] text-[0.72rem] tracking-[0.08em] font-[600] text-[#7E2A23]">
  ...
  return (
    <div
      className="inline-flex items-center gap-2 rounded-full px-4 py-2 ring-1"
      style={{ background: bg, borderColor: ring, boxShadow: `inset 0 0 0 1px ${ring}` }}
    >
      <Clock className="h-3.5 w-3.5" strokeWidth={1.8} style={{ color: fg }} />
      <span
        className="font-[family-name:var(--font-raleway)] text-[0.72rem] tracking-[0.08em] font-[600]"
        style={{ color: fg }}
      >
        Cancels in{" "}
